...
class TestShardingStringRange(tests.utils.TestCase):
    ...
    def setUp(self):
        ...
        expected_shard_mapping_id = tests.utils.make_mapping_result([[1, "RANGE_STRING", "GROUPID1"]])
    ...
    def test_shard_range(self):
        ...
        are working.
        """
        status = self.proxy.sharding.lookup_servers("db1.t1", "a007",  "LOCAL")
        found = False
        ...
        self.assertRaises(_errors.DatabaseError, shard_server.exec_stmt,
                          "INSERT INTO db1.t1 VALUES('c220','Data220')")
        self.assertRaises(_errors.DatabaseError, shard_server.exec_stmt,
                          "INSERT INTO db1.t1 VALUES('z220','Data220')")
        #Inserting a value within the valid range should pass.
        shard_server.exec_stmt(
            "INSERT INTO db1.t1 VALUES('a007','Data007')"
        )

        status = self.proxy.sharding.lookup_servers("db1.t1", "b007",  "LOCAL")
        found = False
        for obtained_server in  self.check_xmlrpc_iter(status):
            if obtained_server['status'] == "PRIMARY" and obtained_server['mode'] == "READ_WRITE":
                found = True
        ...
        #Inserting a row out of range should fail.
        self.assertRaises(_errors.DatabaseError, shard_server.exec_stmt,
                          "INSERT INTO db1.t1 VALUES('a220','Data220')")
        self.assertRaises(_errors.DatabaseError, shard_server.exec_stmt,
                          "INSERT INTO db1.t1 VALUES('d220','Data220')")
        #Inserting a value within the valid range should pass.
        shard_server.exec_stmt(
            "INSERT INTO db1.t1 VALUES('b007','Data007')"
        )

        status = self.proxy.sharding.lookup_servers("db1.t1", "c007",  "LOCAL")
        found = False
        for obtained_server in  self.check_xmlrpc_iter(status):
            if obtained_server['status'] == "PRIMARY" and obtained_server['mode'] == "READ_WRITE":
                found = True
        ...
        self.assertRaises(_errors.DatabaseError, shard_server.exec_stmt,
                          "INSERT INTO db1.t1 VALUES('a220','Data220')")
        self.assertRaises(_errors.DatabaseError, shard_server.exec_stmt,
                          "INSERT INTO db1.t1 VALUES('b220','Data220')")
        #Inserting a value within the valid range should pass.
        shard_server.exec_stmt(
            "INSERT INTO db1.t1 VALUES('c007','Data007')"
        )

        status = self.proxy.sharding.lookup_servers("db1.t1", "d007",  "LOCAL")
        found = False
        for obtained_server in  self.check_xmlrpc_iter(status):
            if obtained_server['status'] == "PRIMARY" and obtained_server['mode'] == "READ_WRITE":
                found = True
        ...
        #Inserting a value within the valid range should pass.
        shard_server.exec_stmt(
            "INSERT INTO db1.t1 VALUES('d007','Data007')"
        )

        status = self.proxy.sharding.lookup_servers("db1.t1", "e007",  "LOCAL")
        found = False
        for obtained_server in  self.check_xmlrpc_iter(status):
            if obtained_server['status'] == "PRIMARY" and obtained_server['mode'] == "READ_WRITE":
                found = True
        ...
        #Inserting a value within the valid range should pass.
        shard_server.exec_stmt(
            "INSERT INTO db1.t1 VALUES('e007','Data007')"
        )
