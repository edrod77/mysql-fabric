...
def get_master_status(server, options=None):
    ...
    :param options: Define how the result is formatted and retrieved.
                    See :meth:`mysql.hub.server.MySQLServer.exec_query`.
    """
    if options is None:
        options = {}
    options["columns"] = True
    options["raw"] = False
    return server.exec_query("SHOW MASTER STATUS", options)
...
def get_master_rpl_users(server, options=None):
    ...
    if options is None:
        options = {}
    options["columns"] = True
    return server.exec_query(_RPL_USER_QUERY, options)
...
def get_master_slaves(server, options=None):
    ...
    if options is None:
        options = {}
    options["columns"] = True
    return server.exec_query("SHOW SLAVE HOSTS", options)

@server_logging
def check_master_rpl_health(server):
    """Check replication health on the master.

    This method checks if the master is set up correctly to operate
    in a replication environment. It returns a tuple with a bool to
    indicate if health is Ok (True), and a list that contains errors
    encountered during the checks, if there is any. Basically, it
    checks if the master is alive and kicking, if the binary log is
    enabled and finally if there is a user that has the `REPLICATION
    SLAVE PRIVILEGE`. One may find in what follows the set of possible
    returned values::

      ret = check_master_rpl_health(server)
...
def get_slave_status(server, options=None):
    ...
    options["columns"] = True
    options["raw"] = False
    return server.exec_query("SHOW SLAVE STATUS", options)

@server_logging
def is_slave_connected(server):
    """Check to see if slave is connected to a master and the IO Thread
    is running.
    """
    ret = get_slave_status(server)
    is_connected = False
    if ret:
        is_connected = ret[0].Slave_IO_Running.upper() == "YES"
    return is_connected

@server_logging
def slave_is_connected_to_master(server):
    ...
    """
    ret = get_slave_status(server)
    is_connected_to = None
    if ret and ret[0].Slave_IO_Running.upper() == "YES":
        is_connected_to = str(ret[0].Master_UUID)
...
def get_num_gtid_behind(gtids, server_uuid=None):
    ...
    for gtid in gtids.split(","):
        ...
        if gtid.find(":") != -1:
            ...
            if not sid:
              raise _errors.DatabaseError("Malformed GTID (%s)." % gtid)
            trx_ids = gtid
...
def get_slave_num_gtid_behind(server, master_gtids, master_uuid=None):
    ...
    if master_gtids == "" and slave_gtids != "":
        raise _errors.DatabaseError("It is not possible to check the lag "
       ...
       gtids = server.exec_query("SELECT GTID_SUBTRACT('%s','%s')" %
...
def start_slave(server, threads=None, wait=False, timeout=None):
    ...
    """
    threads = threads or ()
    server.exec_query("START SLAVE " + ", ".join(threads))
    if wait:
...
def stop_slave(server, threads=None, wait=False, timeout=None):
    ...
    """
    threads = threads or ()
    server.exec_query("STOP SLAVE " + ", ".join(threads))
    if wait:
...
def reset_slave(server, clean=False):
    ...
    server.exec_query("RESET SLAVE %s" % (param))
...
def wait_for_slave_thread(server, timeout=None, wait_for_running=True,
    ...
    achieved. If the timeout period experies prior to achieving the
...
def wait_for_slave(server, binlog_file, binlog_pos, timeout=3):
    ...
    This methods call the MySQL function `SELECT MASTER_POS_WAIT`. If the
    timeout period experies prior to achieving the condition the
    the :class:`mysql.hub.errors.TimeoutError` exception is raised.
    ...
    res = server.exec_query(_MASTER_POS_WAIT %
...
def wait_for_slave_gtid(server, master_gtids, timeout=3):
    ...
    slave catches up. If the timeout period experies prior to achieving
...
def switch_master(slave, master, master_user, master_passwd=None,
    ...
    slave.exec_query("CHANGE MASTER TO " + ", ".join(params))

@server_logging
def check_slave_rpl_health(server, master_uuid, master_status,
                           master_gtid_status, max_delay_behind,
                           max_pos_behind, max_gtid_behind):
    """Check replication health of the slave.

    This method checks to see if the slave is setup correctly to operate
    in a replication environment. It returns a tuple with a bool to
    indicate if health is Ok (True), and a list that contains errors
    encountered during the checks, if there is any. Basically, it
    checks if `SQL_THREAD` and `IO_THREAD` are running, if the slave is
    not lagging behind and is connected to the expected master.
    ...
    if ret:
        ...
        # TODO: Check if this should really be verified.
        if rpl_ok and \
           slave_is_connected_to_master(server).upper() != \
           str(master_uuid).upper():
            errors.append("Not connected to correct master.")
            rpl_ok = False
    else:
